...
> **版本**: v1.0 | **编写日期**: 2026-04-16
> **当前状态**: Stage 1（Bug修复）✅ 完成 | Stage 2（多源抓取 + ChromaDB + 层级主题）✅ 完成
...
- [ ] **Step 3.1** — 新建 `src/core/nlp/trend_analysis.py`（趋势分析工具函数）
- [ ] **Step 3.2** — 修改 `src/core/api/topic_routes.py`，新增 `/api/topics/sunburst` 端点
- [ ] **Step 3.3** — 修改 `src/core/api/topic_routes.py`，新增 `/api/topics/trends` 端点
- [ ] **Step 3.4** — 修改 `src/core/api/topic_routes.py`，完善 `/api/topics/{id}/papers` 支持分页排序
- [ ] **Step 3.5** — 修改 `app.py`，新增"主题地图"标签页（Sunburst 图）
- [ ] **Step 3.6** — 修改 `app.py`，实现主题钻取交互（点击扇区→右侧论文列表联动）
- [ ] **Step 3.7** — 修改 `app.py`，新增"研究趋势"标签页（折线图）
- [ ] **Step 3.8** — 修改 `app.py`，改造搜索流程，加入 4 阶段进度条
- [ ] **Step 3.9** — 修改 `requirements.txt`，确认 `plotly>=5.18.0` 已包含
- [ ] **Step 3.10** — 新建 `src/test_stage3_viz.py`，集成测试可视化 API
...
Stage 3 让系统好用，Stage 4 让系统有深度，Stage 5 让系统更智能——用 LLM 生成自然语言主题名称，并完成面向团队演示的生产部署。

### Stage 5 目标

> 用 LLM 提升智能感，完成生产部署，达到可演示的完整产品状态。
...
| Docker 容器化 | 一键部署，无需手动配置 conda | 低 |
| 单元/集成测试 | 提高代码稳健性 | 低 |
...
### 4.5 功能五：Docker 容器化部署

#### 为什么需要？

方便在导师服务器或其他机器上一键部署，无需手动安装 conda 环境：
...
# 自动完成：
# - 后端 FastAPI 启动（端口 8000）
# - 前端 Streamlit 启动（端口 8501）
# - data/ 和 models/ 目录持久化挂载
```
...
| 文件 | 内容 |
|------|------|
| `docker/Dockerfile.backend` | 后端镜像（Python 3.10 + conda 依赖） |
| `docker/Dockerfile.frontend` | 前端镜像（Python 3.10 + streamlit） |
| `docker-compose.yml` | 编排文件（后端+前端一键启动） |
| `.dockerignore` | 排除 data/、models/、.env 等文件 |
...
| 5.8 | Docker 容器化 | `docker/` 目录 + `docker-compose.yml`（新建） | 4小时 |
| 5.9 | 测试覆盖补充 | `tests/` 目录完善 | 4小时 |
| 5.10 | 文档更新 | `README.md`、`QUICKSTART.md`（修改） | 2小时 |

**Stage 5 合计预估工作量**：约 25.5 小时（3.5 个工作日）
...
| Stage 5 Docker | Stage 3+4（所有功能完成） | — |
...
| 阶段 | 主要交付物 | 预估工作量 | 建议周期 |
|------|-----------|-----------|---------|
| Stage 3 | Sunburst图、趋势图、进度条、主题钻取 | 24.5 小时 | 第3周 |
| Stage 4 | 收藏/笔记、BibTeX导出、相关推荐、引用网络 | 26 小时 | 第4-5周 |
| Stage 5 | LLM命名、缓存、日志、Docker | 25.5 小时 | 第6-7周 |
| 合计 | | 约 76 小时 | 约5周 |
...
| 新建 | `docker/Dockerfile.backend` |
| 新建 | `docker/Dockerfile.frontend` |
