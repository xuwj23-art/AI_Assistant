...
class TopicModeler:
    ...
    def get_hierarchical_topics(self, documents: List[str]) -> pd.DataFrame:
        ...
        参数:
            documents: 原始文档列表（用于计算层级结构）
        
        返回:
            层级主题 DataFrame，包含 Parent_ID, Parent_Name, Topics, Child_Left_ID, Child_Right_ID 等列
        
        说明:
            BERTopic 使用凝聚层次聚类（Agglomerative Hierarchical Clustering）
            自动构建主题树，无需多次训练
        """
        if self.verbose:
            print("生成层级主题结构...")
        ...
        if self.verbose:
            print(f"层级主题生成完成，共 {len(hierarchical_topics)} 个节点")
        
        return hierarchical_topics
    
    def export_hierarchy_json(self, documents: List[str]) -> dict:
        """
        导出层级主题为前端可用的 JSON 格式
        ...
        返回:
            层级结构字典，格式:
            {
                ...
                "children": [
                    {
                        "name": "Topic 0: machine_learning_deep",
                        "value": 150,
                        "topic_id": 0,
                        "children": [...]
                    },
        ...
        """
        hierarchical_topics = self.get_hierarchical_topics(documents)
        ...
        for _, row in topic_info.iterrows():
            ...
            if topic_id != -1:
                # 使用前3个关键词作为主题名
                topic_names[topic_id] = self.get_topic_name(topic_id, max_words=3)
        ...
        # 获取所有叶子主题（最底层的主题）
        leaf_topics = set(topic_info['Topic'].tolist())
        if -1 in leaf_topics:
            leaf_topics.remove(-1)  # 移除噪声主题
        
        for topic_id in sorted(leaf_topics):
            topic_name = topic_names.get(topic_id, f"Topic {topic_id}")
            # 获取该主题的文档数量
            topic_count = len([t for t in self.topic_model.topics_ if t == topic_id])
